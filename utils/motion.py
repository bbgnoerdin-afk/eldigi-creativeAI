...
def generate_device_id():
    ...
    random_hex = ''.join(random.choices('0123456789abcdef', k=16 - len(timestamp_hex)))
    return timestamp_hex + random_hex

def generate_random_token():
    # Menghasilkan 32 karakter hex acak untuk token
    return uuid.uuid4().hex

def get_random_device():
    devices = [
        {"info": "Infinix X6728", "system": "INFINIX,Infinix-X6728,Infinix X6728,Infinix-X6728"},
        {"info": "Samsung SM-S911B", "system": "SAMSUNG,SM-S911B,Samsung Galaxy S23,SM-S911B"},
        {"info": "Xiaomi 2211133G", "system": "XIAOMI,2211133G,Xiaomi 13,2211133G"},
        {"info": "OPPO CPH2521", "system": "OPPO,CPH2521,Oppo Reno 10,CPH2521"},
        {"info": "vivo V2246", "system": "VIVO,V2246,Vivo V27,V2246"},
        {"info": "POCO F5", "system": "POCO,23049PCD8G,POCO F5,23049PCD8G"}
    ]
...
def get_sheader(key):
    try:
        ...
        print(f"Error load sheader: {e}")
        return ""

def get_headers_crushnow(api_type):
    return {
...
async def upload_image_crushnow(img_bytes):
    url_up = "https://crushnow.xyz/controller/v/upload/_diversification/upload"
    headers = get_headers_crushnow("upload")
    files = {'file': ('image.jpg', img_bytes, 'image/jpeg')}
    
    def do_upload():
        return requests.post(url_up, headers=headers, files=files)
        
    try:
        res_up = await asyncio.to_thread(do_upload)
        if res_up.status_code == 200:
            img_url = res_up.json().get("data")
            return img_url
        return None
    except Exception as e:
        print(f"Error upload ke crushnow: {e}")
        return None
...
async def get_video_url_ssstik(tiktok_url):
    ...
    payload = {"id": tiktok_url, "locale": "en", "tt": "aThwYTVl"}
    try:
        response = await async_post(post_url, headers=headers, data=payload)
        match = re.search(r'<a href="([^"]+)"[^>]*class="[^"]*without_watermark[^"]*"', response.text)
...
async def setup_dreamface_user():
    ...
    headers_auth = {"host": "log.dreamfaceapp.com", "content-type": "application/json; charset=utf-8", "user-agent": "okhttp/4.12.0", "X-Forwarded-For": random_ip}
    ...
    headers_login = {"User-Id": uid, "Platform-Type": "ANDROID", "App-Version": "6.25.6", "System-Version": "15", "App-Type": "dreamface_free", "Language": "id", "x-signature": "7F3CDCA50A93309DD4C5EAA1C31116BB", "Content-Type": "application/json;charset=UTF-8", "User-Agent": "okhttp/4.12.0", "X-Forwarded-For": random_ip}
    ...
    try: await async_post(url_login, headers=headers_login, json=payload_login, timeout=15)
    except: pass
    ...
    try: await async_post(url_quota, headers=headers_quota, json=payload_quota, timeout=15)
    except: pass
...
async def submit_motion_task(image_bytes, image_name, tiktok_url, replace_background=False):
    # 1. Compress Image
    compressed_bytes, filename = await asyncio.to_thread(_compress_image_sync_worker, image_bytes, image_name)
    
    # 2. Upload to Crushnow
    final_image_url = await upload_image_crushnow(compressed_bytes)
    if not final_image_url:
        print("[-] Gagal upload gambar ke Crushnow")
        return None, None
...
async def poll_motion_task(uid, task_id, start_time):
    ...
    while True:
        # 1. Cek Timeout Ketat: Batas Waktu 20 Menit (1200 detik)
        ...
        try:
            response = await async_post(url_poll, headers=headers_poll, json=payload_poll, timeout=15)
            res = response.json()
            
            if res.get("status_code") == "THS12140000000":
                anim_list = res.get("data", {}).get("animate_image_list", [])
                
                # Pastikan list tidak kosong sebelum mengecek state
                if anim_list:
                    ...
                    if state in ["success", "completed", "finish"]:
                        ...
                        resp_work = await async_post(url_work, headers=headers_work, json=payload_work, timeout=20)
                        res_work = resp_work.json()
                    ...
                    # Jika status server menyatakan kegagalan (bukan proses antrean)
                    elif state not in ["queue", "processing", "timeout"]:
                        return "failed", f"Task gagal. Ditolak oleh server dengan status: {state}"
                    
                    # Jika state = queue/processing, biarkan berlanjut ke bawah untuk sleep
            else:
                # Jika token hangus atau server down mendadak
                return "failed", f"Koneksi ditolak server (Status Kode: {res.get('status_code')})"
                
        except Exception as e: 
            # ANTI KONEKSI MATI: Jika HTTP Request terputus sedetik, biarkan bot diam lalu mencoba lagi.
            # Bot TIDAK akan crash, melainkan masuk ke fase sleep di bawah.
            pass
        
        # SANGAT PENTING: Selalu eksekusi Sleep 8 detik di akhir iterasi.
        # Ini mencegah CPU server tersedot 100% dan tidak akan memblokir user lain!
